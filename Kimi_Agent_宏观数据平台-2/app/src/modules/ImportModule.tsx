...
import { months, importData, getIndexRange } from '../data/economicData';
...
export function ImportModule() {
  ...
  return (
    <div className="space-y-4">
      <ChartCard title="进口总额同比增速" subtitle={`${dr1.startStr} ~ ${dr1.endStr} | ${DATA_SOURCES.import}`} dateRange={dr1}>
        <ReactECharts option={{
          ...
          series: [{ type: 'line', data: importData.yoy.slice(s1, e1), smooth: true, lineStyle: { color: '#3b82f6', width: 2 }, itemStyle: { color: '#3b82f6' }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(59,130,246,0.15)' }, { offset: 1, color: 'rgba(59,130,246,0)' }] } }, symbol: 'circle', symbolSize: 3 }],
          animationDuration: 500,
        }} style={{ height: 380 }} />
      </ChartCard>

      <div className="grid grid-cols-2 gap-4">
        <ChartCard title="进口分国家/地区占比">
          <ReactECharts option={{
            tooltip: { trigger: 'item' as const, backgroundColor: 'rgba(255,255,255,0.95)', borderColor: '#e2e8f0', textStyle: { color: '#1e293b' } },
            series: [{
              type: 'pie', radius: ['40%', '70%'], center: ['50%', '55%'],
              data: importData.byCountry.map(c => ({ name: c.name, value: c.value })),
              label: { color: '#475569', fontSize: 11, formatter: '{b}\n{d}%' },
              itemStyle: { borderRadius: 4, borderColor: '#fff', borderWidth: 2 },
            }],
            color: ['#3b82f6', '#ef4444', '#f59e0b', '#22c55e', '#8b5cf6', '#06b6d4', '#94a3b8'],
            animationDuration: 500,
          }} style={{ height: 320 }} />
        </ChartCard>

        <ChartCard title="进口分产品类别占比">
          <ReactECharts option={{
            tooltip: { trigger: 'item' as const, backgroundColor: 'rgba(255,255,255,0.95)', borderColor: '#e2e8f0', textStyle: { color: '#1e293b' } },
            series: [{
              type: 'pie', radius: ['40%', '70%'], center: ['50%', '55%'],
              data: importData.byProduct.map(c => ({ name: c.name, value: c.value })),
              label: { color: '#475569', fontSize: 11, formatter: '{b}\n{d}%' },
              itemStyle: { borderRadius: 4, borderColor: '#fff', borderWidth: 2 },
            }],
            color: ['#3b82f6', '#ef4444', '#f59e0b', '#22c55e', '#8b5cf6', '#06b6d4', '#94a3b8'],
