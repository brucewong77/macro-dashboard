...
import { months, creditData, getIndexRange, blankUnpublished } from '../data/economicData';
...
export function CreditModule() {
  const dr1 = useChartDateRange(2023, 4);
  ...
  const creditHouseholdShort = useMemo(() => blankUnpublished(fm1, creditData.householdShort.slice(s1, e1), 'sf'), [fm1, s1, e1]);
  const creditHouseholdLong = useMemo(() => blankUnpublished(fm1, creditData.householdLong.slice(s1, e1), 'sf'), [fm1, s1, e1]);
  const creditEnterpriseShort = useMemo(() => blankUnpublished(fm1, creditData.enterpriseShort.slice(s1, e1), 'sf'), [fm1, s1, e1]);
  const creditEnterpriseLong = useMemo(() => blankUnpublished(fm1, creditData.enterpriseLong.slice(s1, e1), 'sf'), [fm1, s1, e1]);
  const creditBillFinancing = useMemo(() => blankUnpublished(fm1, creditData.billFinancing.slice(s1, e1), 'sf'), [fm1, s1, e1]);
    ...
    <div className="space-y-4">
      <ChartCard title="人民币贷款结构（万亿元）" subtitle={`${dr1.startStr} ~ ${dr1.endStr} | ${DATA_SOURCES.credit}`} dateRange={dr1}>
        <ReactECharts option={{
          ...
          series: [
            { name: '住户短期', type: 'bar', stack: 'household', data: creditHouseholdShort, itemStyle: { color: '#f59e0b' }, barWidth: '60%' },
            { name: '住户中长期', type: 'bar', stack: 'household', data: creditHouseholdLong, itemStyle: { color: '#ef4444' } },
            { name: '企业短期', type: 'bar', stack: 'enterprise', data: creditEnterpriseShort, itemStyle: { color: '#3b82f6' } },
            { name: '企业中长期', type: 'bar', stack: 'enterprise', data: creditEnterpriseLong, itemStyle: { color: '#22c55e' } },
            { name: '票据融资', type: 'bar', data: creditBillFinancing, itemStyle: { color: '#8b5cf6' } },
          ],
